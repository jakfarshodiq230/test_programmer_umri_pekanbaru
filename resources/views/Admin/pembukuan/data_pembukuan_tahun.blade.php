...
@section('scripts')
    <script>
        ...
        $('#datatables-ajax-bulan').DataTable({
            ...
            destroy: true,
            responsive: true,
            ajax: '{{ url('pembukuan/data_pembukuan_tahun_periode') }}',
            columns: [{
                    data: null,
                    name: 'nomor',
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        return row.judul_periode + '<br>[' + row.tahun_pembukuan + ']';
                    }
                },
                {
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `
                                <button class="btn btn-sm btn-primary lihatBtn" data-bs-toggle="tooltip" data-bs-placement="top" title="Lihat Data" data-id="${row.id_pembukuan}" data-periode ="${row.id_periode}" data-tahun ="${row.tahun_pembukuan}"><i class="fas fa-eye"></i></button>
                            `;
                    }
                }
            ...
            buttons: [{
                ...
                action: function(e, dt, node, config) {
                    ...
                    $.ajax({
                        url: '{{ url('pembukuan/data_pembukuan_tahun_periode') }}',
                        type: 'GET',
                        xhr: function() {
        ...
        $('#saveBtn').on('click', function() {
            var url = '{{ url('pembukuan/store_pembukuan_tahun') }}';
            $.ajax({
                url: url,
                method: 'POST',
        ...
        $(document).on('click', '.lihatBtn', function() {
            var idPembukuan = $(this).data('id');
            var idPeriode = $(this).data('periode');
            var idTahun = $(this).data('tahun');
            // Construct the URL based on the id and tggl
            var url = '{{ url('pembukuan/data_pembukuan_tahun_periode_rincian') }}/' + idPembukuan + '/' + idPeriode+'/'+idTahun;
